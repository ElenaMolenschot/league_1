...
SELECT
  ...
  SAFE_CAST(NULLIF(GA_Shot_Stopping, 'NA') AS INT64) AS GA_Shot_Stopping,
  SAFE_CAST(NULLIF(Saves_Shot_Stopping, 'NA') AS INT64) AS Saves_Shot_Stopping,
  SAFE_CAST(NULLIF(REGEXP_REPLACE(Save_percent_Shot_Stopping, r'[%]', ''), 'NA') AS FLOAT64) AS Save_percent_Shot_Stopping,
  ...
  SAFE_CAST(NULLIF(REGEXP_REPLACE(Cmp_percent_Launched, r'[%]', ''), 'NA') AS FLOAT64) AS Cmp_percent_Launched,
  SAFE_CAST(Att_GK_Passes AS INT64) AS Att_GK_Passes,
  SAFE_CAST(Thr_Passes AS INT64) AS Thr_Passes,
  SAFE_CAST(NULLIF(REGEXP_REPLACE(Launch_percent_Passes, r'[%]', ''), 'NA') AS FLOAT64) AS Launch_percent_Passes,
  SAFE_CAST(AvgLen_Passes AS FLOAT64) AS AvgLen_Passes,
  SAFE_CAST(Att_Goal_Kicks AS INT64) AS Att_Goal_Kicks,
  SAFE_CAST(NULLIF(REGEXP_REPLACE(Launch_percent_Goal_Kicks, r'[%]', ''), 'NA') AS FLOAT64) AS Launch_percent_Goal_Kicks,
  SAFE_CAST(NULLIF(REGEXP_REPLACE(AvgLen_Goal_Kicks, r'[%]', ''), 'NA') AS FLOAT64) AS AvgLen_Goal_Kicks,
  SAFE_CAST(Opp_Crosses AS INT64) AS Opp_Crosses,
  SAFE_CAST(NULLIF(Stp_Crosses, 'NA') AS INT64) AS Stp_Crosses,
  SAFE_CAST(NULLIF(REGEXP_REPLACE(Stp_percent_Crosses, r'[%]', ''), 'NA') AS FLOAT64) AS Stp_percent_Crosses,
  SAFE_CAST(NULLIF(Player_NumOPA_Sweeper, 'NA') AS INT64) AS Player_NumOPA_Sweeper,
  SAFE_CAST(NULLIF(REGEXP_REPLACE(AvgDist_Sweeper, r'[%]', ''), 'NA') AS FLOAT64) AS AvgDist_Sweeper
